    ...
    def process_heap_dump(self, file_path: str, memory_limit: str = "11g") -> Dict:
        ...
        try:
            ...
            cmd = [
                ...
                self.docker_image,
                filename, memory_limit, "suspects,overview"
            ]
...
class HeapDumpHandler(FileSystemEventHandler):
    ...
    def _extract_machine_name(self, filename: str) -> str:
        ...
        # Try to extract machine name from common patterns
        # Example patterns: machine1_20231215.hprof, heap_machine2_dump.bin
        patterns = [
            r'^([^_]+)_.*',  # machine_timestamp pattern
            r'.*_([^_]+)_.*', # prefix_machine_suffix pattern
    ...
    def get_reports_by_machine(self, machine_name: str) -> List[HeapDumpReport]:
        ...
        with self.lock:
            return [
                ...
                if report.machine_name == machine_name
            ]
    ...
    def get_reports_by_date(self, date_str: str) -> List[HeapDumpReport]:
        ...
        try:
            ...
            with self.lock:
                return [
                    report for report in self.reports.values()
                    if report.timestamp.date() == target_date
                ]
        except ValueError:
            return []
...
def create_app(report_manager: ReportManager) -> Flask:
    ...
    # HTML template for report listing
    ...
    <head>
        ...
        <style>
            ...
            .report-link { color: blue; text-decoration: underline; cursor: pointer; }
        </style>
    </head>
    <body>
        ...
        <table>
            ...
            <tr>
                ...
                <td>{{ "%.2f"|format(report.processing_time) }}s if report.processing_time else 'N/A' }}</td>
                <td>
                    ...
                    {% if report.error_message %}
                        <br><small style="color: red;">{{ report.error_message }}</small>
                    {% endif %}
                </td>
            </tr>
            {% endfor %}
...
def main():
    ...
    parser.add_argument('--docker-image', default='docker.bintray.io/jfrog/auto-mat', 
                       help='Auto-MAT Docker image')
